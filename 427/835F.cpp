...
struct region { ll idx; vector<ll> v; ll imp; ll color;};
bool operator< (const region &a, const region &b) { return a.imp < b.imp; }
...
struct solver {
    ...
    void dealWithImportance() {
        for (auto &r : regions) {
            r.imp = 0;
            for (auto &&nn : r.v) r.imp = r.imp | (1 << nn); 
        }
